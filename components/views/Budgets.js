...
import { formatINR, groupBy, sum, classNames } from '@/lib/utils';
import { useToast } from '@/components/ui/Toast';

export default function Budgets({ store, monthKey, setMonthKey }) {
  const { categories, transactions, budgetFor, setBudget, addCategory, deleteCategory, monthlyTotalFor, setMonthlyTotal } = store;
  ...
  const savedTotal = monthlyTotalFor(monthKey);
  const monthlyTotal = savedTotal !== null ? savedTotal : allocated;
  ...
  async function handleClearAll() {
    if (!confirm('Set every category budget to ₹0 for this month? You can then type your own numbers in from scratch.')) return;
    try {
    ...
    <div className="space-y-5 animate-rise">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-2xl font-semibold text-paper-100">Monthly budgets</h1>
        </div>
      ...
      <Panel
        ...
        eyebrow="set once, split across categories below"
        action={
          <button
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-ink-border">
          <div>
            <div className="text-[10px] uppercase tracking-wide text-paper-500 font-mono">Total budget</div>
      ...
      <Panel>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-[11px] uppercase tracking-wide text-paper-500 font-mono">Left of total budget</div>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {categories.map((c) => {
          ...
          const budget = budgetFor(monthKey, c.id);
