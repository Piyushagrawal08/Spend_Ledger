...
// Everything above is *relative* — what you spent, what is left of an
// allocation. None of it knows how much money exists. These two do.
//
// The balance is anchored on `openingBalance` (what you held before the oldest
// entry, from settings) and moves with `netFlow`: every credit in, every debit
// out. Refunds count as cash in here, unlike in `netSpend` where they come off
// a category — the money genuinely came back to you either way.
//
// Read-side only, like budget carry-forward and rollover. No cycle's balance is
// ever stored, so a back-dated entry re-derives every later figure on the next
// render instead of leaving a stale total behind.
//
// DO NOT feed these a category-filtered list. Income carries no category, so
// any slice drops every credit and the balance silently becomes a pure spend
// total. They take the whole ledger, always.

/** Cash held immediately before `dateISO` — the date itself does not count. */
export function cashBalanceBefore(txs, dateISO, openingBalance = 0) {
  let balance = Number(openingBalance) || 0;
  for (const t of txs) {
    if (!t?.date || t.date >= dateISO) continue;
    balance += isCredit(t) ? (Number(t.amount) || 0) : -(Number(t.amount) || 0);
  }
  return balance;
}

/**
 * The cash story of one cycle: what carried in, what moved, what it closes on.
 *
 *   opening + netFlow = closing
 *
 * `opening` is the previous cycle's `closing` by construction — it is the same
 * walk over the same ledger, one cycle-boundary earlier — so the chain can
 * never disagree with itself the way two separately stored figures could.
 */
export function cycleCashSummary(txs, cycleKey, resetDay = DEFAULT_CYCLE_RESET_DAY, openingBalance = 0) {
  const start = cycleStartDate(cycleKey, resetDay);
  const opening = cashBalanceBefore(txs, start, openingBalance);
  const inCycle = filterCycle(txs, cycleKey, resetDay);
  const { debits, credits, income, refunds, netFlow: flow } = ledgerTotals(inCycle);
  return {
    opening,
    debits,
    credits,
